...
def build(bld):
    ...
    bld.program(name='fuzz',
                ...
                cxxflags = ['-fsanitize=address,fuzzer-no-link', '-DCUSTOM_MUTATOR'])
                #,'-fprofile-instr-generate', '-fcoverage-mapping'])
    bld.program(name='fuzzData',
                target='daemon/fuzzerData',
                source='daemon/fuzzerData.cpp',
                use='daemon-objects SYSTEMD',
                ldflags = ['-fsanitize=address,fuzzer'],
                #'-fprofile-instr-generate'],
                cxxflags = ['-fsanitize=address,fuzzer-no-link', '-DCUSTOM_MUTATOR'])
